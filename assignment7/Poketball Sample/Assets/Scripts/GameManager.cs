...
public class GameManager : MonoBehaviour
{
    ...
    void SetupBalls()
    {
        ...
        float spacing = (BallRadius * 2f) + RowSpacing;
        //float rowSpacing = BallRadius + (RowSpacing / 2);
        int k = 0;
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j <= i; j++)
            {
                ...
                ballRenderer.material = Resources.Load<Material>($"Materials/ball_{k}");
                ball.name = $"ball_{k}";

                ball.transform.position = new Vector3(StartPosition.x + (j - (i / 2f)) * spacing, StartPosition.y, StartPosition.z - (i * (spacing)));
    ...
    void CamMove()
    {
        ...
        CamObj.transform.position = new Vector3 (CamObj.transform.position.x, 15f, CamObj.transform.position.z);
    ...
    void ShootBallTo(Vector3 targetPos)
    {
        ...
        targetPos.y = PlayerBall.transform.position.y;
